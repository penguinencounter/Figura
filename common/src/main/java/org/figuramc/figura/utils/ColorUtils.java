...
public class ColorUtils {
    ...
    public interface ThemeProvider {
        ...
        FiguraVec3 stop(@Range(from = 0, to = N_STOPS - 1) int idx);
    }

    public static class HueTheme implements ThemeProvider {
        ...
        private final FiguraVec3[] stops = new FiguraVec3[N_STOPS];

        public HueTheme(double hue) {
            this.hue = hue;
            for (int i = 0; i < N_STOPS; i++) stops[i] = generate(i);
        }
        ...
        public FiguraVec3 stop(@Range(from = 0, to = N_STOPS - 1) int idx) {
            return stops[idx].copy();
        }
    }

    public static final ThemeProvider GRAYSCALE = new ThemeProvider() {
        // oklch C = 0, same L as in HueTheme
        private final double[] L = HueTheme.L;
        ...
        private FiguraVec3 generate(@Range(from = 0, to = N_STOPS - 1) int idx) {
            return MathUtils.clamp(
                    ColorUtils.oklabToRGB(FiguraVec3.of(L[idx], 0, 0)),
                    0d,
                    1d
            );
        }
    ...
    static {
        // eager init so assertions get checked faster
        Class<HueTheme> ignored = HueTheme.class;
    }
}
