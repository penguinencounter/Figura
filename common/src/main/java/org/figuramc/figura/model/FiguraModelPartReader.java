...
public class FiguraModelPartReader {
    ...
    public static FiguraModelPart read(
        ...
        String[] collections = partCompound.contains("cn") ? partCompound.getList("cn", Tag.TAG_STRING).stream()
                .map(Tag::getAsString).toArray(String[]::new) : null;
        byte[] collectionInfo = partCompound.contains("pr") ? partCompound.getByteArray("pr") : null;
        String savedUUID = partCompound.contains("nr") ? partCompound.get("nr") instanceof IntArrayTag list ? UUIDUtil.uuidFromIntArray(
                list.getAsIntArray()).toString() : partCompound.get("nr").getAsString() : null;
        FiguraModelPart result = new FiguraModelPart(
                owner, name, savedUUID, customization, vertices,
                children, collections, collectionInfo, formatVersion
        );
        ...
        if (partCompound.contains("anim")) {
            ...
            for (Tag tag : nbt) {
                ...
                if (!compound.contains("id") || !compound.contains("data") || (animation = owner.animations.get(compound.getInt(
                        "id"))) == null)
                    continue;
    ...
    private static Pair<FiguraVec3, String[]> parseKeyframeData(CompoundTag keyframeNbt, String tag) {
        ...
        if (!floatList.isEmpty()) {
            ...
            return Pair.of(
                    null,
                    new String[]{stringList.getString(0), stringList.getString(1), stringList.getString(2)}
            );
        }
    }
